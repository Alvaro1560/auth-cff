...
// GetByID consulta un registro por su ID
func (s *orcl) getByEmail(email string) (*VerificationEmail, error) {
	const psqlGetByEmail = `SELECT id , email, verification_code, identification, verification_date, created_at, updated_at FROM auth.verification_email WITH (NOLOCK) WHERE email = :1 `
	mdl := VerificationEmail{}
...
// GetByID consulta un registro por su ID
func (s *orcl) getByIdentification(identification string) (*VerificationEmail, error) {
